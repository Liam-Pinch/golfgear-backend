...
router.post('/', authenticateToken, (req, res) => {
    ...
    db.get('SELECT * FROM baskets WHERE user_id = ? AND product_id = ?', [userId, product_id], (err, row) => {
        ...
        if (row){
            ...
            db.run('UPDATE baskets SET quantity = ? WHERE id = ?', [newQty, row.id], function(err){err.message})
            res.json({message: 'Basket updated', basket: {id: row.id, user_id: userId, product_id, quantity: newQty}});

        }else{
            db.run('INSERT INTO baskets (user_id, product_id, quantity) VALUES (?, ?, ?)'), [userId, product_id, quantity], function(err){
                if (err) return res.status(500).json({message: 'Database error', error: err.message});
                res.status(201).json({message: 'Product added to basket', basket: {id: this.lastID, user_id: userId, product_id, quantity}});
            }
        }
